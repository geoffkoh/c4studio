...
| Diagram title and legend | ✅ (title bar, legend of used styles) | ✅ | Derived once in `graph/view_graph.py` — entries name the style tag that produced them (`Datastore`) rather than the C4 kind, with externals called out. Drawn by the headless renderer *and* the viewer, where they are nodes rather than Panels so PNG/SVG export captures them (PP-99). Element styles only; relationship styling is not wired up |
