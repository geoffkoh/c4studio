---
name: release
description: Cut a pystructurizr release — bump the version, rebuild the committed frontend bundle, build and publish the wheel to PyPI as pystructurizr-studio, and package the VS Code extension as a .vsix. Use when the user wants to release, publish, or ship a new version.
disable-model-invocation: false
argument-hint: "[patch|minor|major] [--python-only|--vscode-only]"
---

# Cut a pystructurizr Release

Two artifacts ship independently and carry **separate version numbers**:

| Artifact | Version lives in | Published to |
| --- | --- | --- |
| Python package `pystructurizr-studio` | `pyproject.toml` → `project.version` | PyPI |
...
The wheel ships `src/pystructurizr/webapp/static/` so end users never need Node.
...
```bash
curl -s https://pypi.org/pypi/pystructurizr-studio/json | \
  python3 -c "import json,sys; d=json.load(sys.stdin)['info']; print(d['name'], d['version'])"

uvx --isolated --from pystructurizr-studio==<version> pystructurizr --version
```
...
`editors/vscode/pystructurizr-vscode-<version>.vsix`. Install it locally to
verify:

```bash
"/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code" \
  --install-extension editors/vscode/pystructurizr-vscode-<version>.vsix
```

The extension resolves its Python backend automatically, bootstrapping via `uv`
when pystructurizr isn't already importable (PP-68) — check that path works from
